# Create New Copilot Skill

STARTER_CHARACTER = 📚🧩

## Description

Create a GitHub Copilot skill.

Skills are a context management mechanism. They package knowledge Copilot needs for specific tasks while keeping context lean through progressive disclosure:
...
Run the update script to fetch the latest Copilot skill docs:
```bash
./update-docs-copilot
```

### 2. Learn Skill Patterns
Read the official documentation in `docs/knowledge/copilot-skill-docs/`:
- `about-agent-skills.md` - What skills are, core concepts and architecture
- `create-skills.md` - Implementation syntax, structure, usage patterns
- `customization-cheat-sheet.md` - Customization options, proven patterns, guidelines

### 3. Clarify the Goal
Ask the user:
- What specific task should Copilot be able to do?
- Think about what Copilot does NOT already know that this skill needs to provide. Show it as a suggestion to the user.
...
The description is the primary trigger mechanism — Copilot uses it to decide when to activate a skill from potentially 100+ installed skills. It must be lean and precise.
...
- Concise instructions. Assume Copilot is smart, but help guide and focus it by providing good order and progressive disclosure.
...
Re-read `docs/knowledge/copilot-skill-docs/create-skills.md` and `customization-cheat-sheet.md`. Compare to what you created:
- Does the description include clear trigger words?
- Is the body concise? Remove anything Copilot already knows.
...
```bash
./skills copilot install [skill-name]
```

**Project (copy, shared via git):**
```bash
./skills copilot local install [skill-name]
```

Check status with `./skills copilot status` or `./skills copilot local status`.

Restart the relevant Copilot interface to load the skill.

### 11. Test
- Restart the relevant Copilot interface to load the skill
- Ask Copilot to do a task that should trigger the skill
- Verify: Does it trigger? Does Copilot follow instructions correctly?
- Try edge cases

### 12. Iterate
- Skill doesn't trigger → improve description with better trigger words
- Copilot misses steps → make instructions more prominent
- Too verbose → remove what Copilot already knows
